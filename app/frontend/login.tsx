...
export default function Login(props: any) {
  const { login, email, senha, setEmail, setSenha } = props;
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-[#FBFBFA] font-sans text-[#2D263B] selection:bg-[#EAE4F2] selection:text-[#2D263B]">
      ...
      <header className="relative z-10 px-6 pt-16 sm:px-12 md:px-16 md:pt-20 text-center">
        ...
        >
          Mantenha o histórico de seus pedidos
        </motion.p>
      </header>
      ...
      <main className="relative z-10 flex flex-1 items-center justify-center px-6 py-12">
        ...
        >
          ...
          <form onSubmit={login} className="relative flex flex-col gap-4 text-center">
            ...
            <div className="mt-4 flex flex-col gap-3 text-left">
              ...
              {/* Botão para redirecionar ao Cadastro (Usando type="button" para evitar conflito com o formulário) */}
              <button
                type="button"
                onClick={() => navigate("/cadastro")}
                className="group relative inline-flex items-center justify-center rounded-full border border-[#EAE4F2] bg-white/70 px-6 py-3.5 text-sm font-medium text-[#2D263B] shadow-sm backdrop-blur-md transition-all duration-300 hover:border-[#D5CBE5] hover:bg-white hover:shadow-md cursor-pointer"
              >
                Fazer cadastro
                <span
                  aria-hidden="true"
